        ...
        <tbody>
            @foreach($trainSchedules as $schedule)
                <tr>
                    ...
                    <td>{{ $schedule->time }}</td>
                    <td>
                        <a href="{{ route('edit.train.schedule', $schedule->id) }}" class="edit-btn">Edit</a>


                        <form action="{{ route('delete.train.schedule', $schedule->id) }}" method="post" style="display: inline-block;">
                            @csrf
                            @method('DELETE')
